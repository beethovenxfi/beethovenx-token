...
import "./ReliquaryEventsMock.sol";
    ...
    ReentrancyGuard
{
    ...
    function emergencyWithdraw(uint relicId) external override nonReentrant {
        ...
        emit ReliquaryEventsMock.EmergencyWithdraw(poolId, amount, to, relicId);
    }
        ...
        returns (uint id)
    {
        ...
        position.poolId = pid;
        _deposit(amount, id);
        emit ReliquaryEventsMock.CreateRelic(pid, to, id);
    }
    ...
    function merge(uint fromId, uint toId) public virtual override nonReentrant {
        ...
        emit ReliquaryEventsMock.Merge(fromId, toId, fromAmount);
    }
    ...
    function _updateLevel(uint relicId) internal returns (uint newLevel) {
        ...
        if (position.level != newLevel) {
            ...
            emit ReliquaryEventsMock.LevelChanged(relicId, newLevel);
        }
    }
