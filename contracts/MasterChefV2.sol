...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    struct UserInfo {
        uint256 amount;
        // amount of rewards you are not eligible for since the start of this farm
        int256 rewardDebt;
    }
    ...
    struct PoolInfo {
        // accumulated rewarded sushi per LP token (?)
    ...
    /// @notice Info of each user per pool that stakes LP tokens.
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        // total LP token supply of this pools LP
        uint256 lpSupply = lpToken[_pid].balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            // blocks since last reward
            uint256 blocks = block.number.sub(pool.lastRewardBlock);
            // we have a fixed number of SUSHI tokens released per block, each pool gets his fraction based on the pool.allocPoint
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                // blocks since last reward
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                // total sushi rewards for this pool based on pool.allocPoint
                uint256 sushiReward = blocks.mul(sushiPerBlock()).mul(pool.allocPoint) / totalAllocPoint;
