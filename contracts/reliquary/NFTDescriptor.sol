...
contract NFTDescriptor is INFTDescriptor {
    ...
    uint256 private constant GRAPH_WIDTH = 210;
    uint256 private constant GRAPH_HEIGHT = 30;

    // TODO: testing account, not ipfs to be used in production
    string private constant IPFS =
        "https://gateway.pinata.cloud/ipfs/QmaayJ6EGM4JmK8P4eenopFrvExn6cwpeTapxti85ETJP4/";
    uint256 private constant NUM_CHARACTERS = 3;
        ...
        returns (string memory uri)
    {
        ...
        string memory amount = generateDecimalString(
            position.amount,
            IERC20Values(underlying).decimals()
        );
        string memory pendingOath = generateDecimalString(
            reliquary.pendingOath(relicId),
            18
        );
        uint256 maturity = (block.timestamp - position.entry) / 1 days;

        uint256 characterId = uint256(
            keccak256(abi.encodePacked(relicId, address(reliquary)))
        ) % NUM_CHARACTERS;
    ...
    function generateImageText(
        ...
        text = string.concat(
            ...
            pendingOath,
            ' OATH</text><text x="50%" y="380" class="bit" style="font-size: 8">MATURITY:',
            maturity.toString(),
            " DAY",
            (maturity == 1) ? "" : "S",
            "</text>"
    ...
    function generateTextFromToken(
        ...
        text = string.concat(
            '<text x="50%" y="340" class="bit" style="font-size: 8">AMOUNT:',
            amountString
        );
    }
    ...
    function generateBars(uint256 level, LevelInfo memory levelInfo)
        internal
        pure
        returns (string memory bars)
    {
        uint256 highestAllocPoint = levelInfo.allocPoint[0];
        for (uint256 i = 1; i < levelInfo.allocPoint.length; i++) {
            if (levelInfo.allocPoint[i] > highestAllocPoint) {
                highestAllocPoint = levelInfo.allocPoint[i];
            }
        }

        uint256 barWidth = (GRAPH_WIDTH * 10) / levelInfo.allocPoint.length;
        ...
        for (uint256 i; i < levelInfo.allocPoint.length; i++) {
            uint256 barHeight = (levelInfo.allocPoint[i] * GRAPH_HEIGHT) /
                highestAllocPoint;
            bars = string.concat(
                bars,
                '<rect x="',
