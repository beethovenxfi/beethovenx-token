...
describe("BeethovenxMasterChef", function () {
  before(async function () {
    await prepare(this, ['BeethovenxMasterChef', 'BeethovenxToken', 'ERC20Mock', 'RewarderMock', 'RewarderBrokenMock'])
    await deploy(this, [
  ...
  beforeEach(async function () {
    await deploy(this, [
      ["beetx", this.BeethovenxToken],
    ])

    await deploy(this,
      [["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
        ['chef', this.BeethovenxMasterChef, [this.beetx.address, this.dev.address, this.treasury.address, getBigNumber(100),0, 200, 200]],
    ])

    await this.beetx.transferOwnership(this.chef.address)
    // await this.chef.add(100, this.lp.address, ethers.constants.AddressZero)
