...
describe('TimeBasedMasterChefRewarder', function () {
    ...
    it('only allows setting of reward token once', async () => {
        ...
        await expect(rewarder.initializeRewardToken(anotherRewardToken.address)).to.be.revertedWith(
    ...
    it('sets rewards per second', async () => {
        ...
        await expect(rewarder.setRewardPerSecond(updatedRewardsPerSecond))
            .to.emit(rewarder, 'LogRewardPerSecond')
            .withArgs(updatedRewardsPerSecond)
    ...
    it('rewards only start accruing after deposit', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))
        ...
        await lpToken.transfer(alice.address, bn(1000))

        //start rewarder, time 0
        await rewarder.setRewardPerSecond(rewardsPerSecond)
        const beforeDeposit = await latest()

        // we advance by 100 seconds
        await advanceToTime(beforeDeposit.toNumber() + 100)

        // add LP
        setAutomineBlocks(false)
        await lpToken.connect(alice).approve(chef.address, bn(50))
        await chef.connect(alice).deposit(0, bn(50), alice.address)
        advanceBlock()
        const afterDeposit = await latest()
        ...
        // we advance by 100 seconds
        await advanceToTime(afterDeposit.toNumber() + 100)
        ...
        expect(await rewardToken.balanceOf(alice.address)).not.to.equal(rewardsPerSecond.mul(200))
        expect(await rewardToken.balanceOf(alice.address)).to.equal(rewardsPerSecond.mul(100))
    })

    it('stops rewards on deposit when totalPendingRewards greater equal remainingRewardToken', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))

        const rewardsPerSecond = bn(1)
        const rewarder = await deployRewarder()
        await rewarder.initializeRewardToken(rewardToken.address)
        ...
        await lpToken.transfer(alice.address, bn(100))
        await rewardToken.transfer(rewarder.address, bn(100))
        ...
        //reward rate unchanged
        await expect(await rewarder.rewardPerSecond()).to.be.equal(rewardsPerSecond)

        const afterDeposit = await latest()
        // we advance by 98 seconds use all rewards, still unchanged
        await advanceToTime(afterDeposit.toNumber() + 98)
        await lpToken.connect(alice).approve(chef.address, bn(10))
        await chef.connect(alice).deposit(0, bn(10), alice.address)
        advanceBlock() // advance 1 second
        await expect(await rewarder.rewardPerSecond()).to.be.equal(rewardsPerSecond)

        await lpToken.connect(alice).approve(chef.address, bn(10))
        await chef.connect(alice).deposit(0, bn(10), alice.address)
        advanceBlock() // advance 1 second
        await expect(await rewarder.rewardPerSecond()).to.be.equal(bn(0))
    })

    it('stops rewards on withdrawAndHarvest when totalPendingRewards greater equal remainingRewardToken', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))
        ...
        //reward rate unchanged
        await expect(await rewarder.rewardPerSecond()).to.be.equal(rewardsPerSecond)
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(1))

        const afterDeposit = await latest()
        // we advance by 98 seconds use all rewards, still unchanged
        await advanceToTime(afterDeposit.toNumber() + 97, true) // time 98
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(98))
        await chef.connect(alice).withdrawAndHarvest(0, bn(10), alice.address)
        advanceBlock() // advance 1 second time 99
        await expect(await rewarder.rewardPerSecond()).to.be.equal(rewardsPerSecond)

        advanceBlock() // advance 1 second time 100
        await chef.connect(alice).withdrawAndHarvest(0, bn(10), alice.address)
        advanceBlock() // advance 1 second
        await expect(await rewarder.rewardPerSecond()).to.be.equal(bn(0))
    })

    it('all rewards can be harvested after rate is set to 0 automatically', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))
        ...
        await lpToken.transfer(alice.address, bn(100))
        await rewardToken.transfer(rewarder.address, bn(3))

        //start rewarder, time 0
        await rewarder.setRewardPerSecond(rewardsPerSecond)
        let pendingRewards = await rewarder.totalPendingRewards()

        console.log(formatUnits(pendingRewards, 18))
        await lpToken.connect(alice).approve(chef.address, bn(100))
        await chef.connect(alice).deposit(0, bn(50), alice.address)
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(0))
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(await rewarder.pendingToken(0, alice.address))

        advanceBlock() // 1 accumulated reward
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(1))
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(await rewarder.pendingToken(0, alice.address))

        await chef.connect(alice).harvest(0, alice.address) // 2 accumulated rewards
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(0))
        await expect(await rewardToken.balanceOf(rewarder.address)).that.be.equal(bn(1))
        await expect(await rewardToken.balanceOf(alice.address)).that.be.equal(bn(2))
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(await rewarder.pendingToken(0, alice.address))

        await chef.connect(alice).deposit(0, bn(50), alice.address) // 3 accumulated rewards
        // await expect(await rewarder.getTotalPendingRewards()).to.be.equal(bn(1))
        // await expect(await rewardToken.balanceOf(rewarder.address)).that.be.equal(bn(1))
        // await expect(await rewardToken.balanceOf(alice.address)).that.be.equal(bn(2))
        await expect(await rewarder.getTotalPendingRewards()).to.be.equal(await rewarder.pendingToken(0, alice.address))
    ...
    it('pendingToken equals totalPendingRewards for one user joining after reward start', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))
    ...
    it('totalPendingRewards is 0 after all LPs leave and stays 0', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000))
        ...
        await chef.connect(alice).deposit(0, bn(50), alice.address)
        advanceBlock()
        await expect(await rewarder.totalPendingRewards()).to.be.equal(await rewarder.pendingToken(0, alice.address))
        await chef.connect(alice).withdrawAndHarvest(0, bn(100), alice.address)
        await expect(await rewarder.totalPendingRewards()).to.be.equal(0)

        const afterWithdraw = await latest()
        // we advance by 98 seconds use all rewards, still unchanged
        await advanceToTime(afterWithdraw.toNumber() + 50)
        await expect(await rewarder.totalPendingRewards()).to.be.equal(0)
    })
    ...
    it('only masterchef can call onBeetsReward hook', async () => {
        ...
        await expect(chef.connect(alice).deposit(0, bn(100), alice.address)).not.to.be.reverted
        await expect(rewarder.onBeetsReward(0, bob.address, bob.address, 10, bn(100))).to.be.revertedWith(
    ...
    it('rejects if anyone else than owner calls shutdown', async () => {
        ...
        await expect(rewarder.connect(alice).shutDown(bob.address)).to.be.revertedWith(
...
function formatFixed(pendingRewards: BigNumber, arg1: number): any {
