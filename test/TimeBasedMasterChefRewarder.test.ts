...
describe('TimeBasedMasterChefRewarder', function () {
    ...
    it('change rewarder contract of running pool', async () => {
        const lpToken = await deployERC20Mock('LPToken', 'LPT', bn(10_000))
        const rewardToken = await deployERC20Mock('Token 1', 'T1', bn(10_000), 6)

        const rewardToken2 = await deployERC20Mock('Token 2', 'T2', bn(10_000), 6)
        const rewarder2 = await deployRewarder()
        ...
        // prepare new rewarder
        await setAutomineBlocks(false)
        await rewarder2.initializeRewardToken(rewardToken2.address)
        await rewarder2.setRewardPerSecond(rewardsPerSecond)
        await rewardToken2.transfer(rewarder2.address, bn(10_000))
        await rewarder2.add(0, 100)
        await setAutomineBlocks(true)
        await advanceBlock()

        const before = await latest()

        // we advance by 100 seconds, simulate accrued rewards from old rewarder, deposit 0 tokens for all users to trigger harvest, shutdown rewarder
        await advanceToTime(before.toNumber() + 100)
        await setAutomineBlocks(false)
        await chef.connect(carol).deposit(0, 0, bob.address)
        await chef.connect(carol).deposit(0, 0, alice.address)
        await rewarder.shutDown(carol.address)
        await setAutomineBlocks(true)
        await advanceBlock()

        // add one second from new rewarder preparation as it also minted a block
        const aliceRewardTokenAmount = rewardsPerSecond.mul(101).mul(3).div(4)
        const bobRewardTokenAmount = rewardsPerSecond.mul(101).div(4)

        expect(await rewardToken.balanceOf(alice.address)).to.equal(aliceRewardTokenAmount)
        expect(await rewardToken.balanceOf(bob.address)).to.equal(bobRewardTokenAmount)

        expect(await rewarder.pendingToken(0, alice.address)).to.equal(bn(0))
        expect(await rewarder.pendingToken(0, bob.address)).to.equal(bn(0))

        console.log(`Old rewarder successfully shut down`)

        // set new rewarder and checkpoint all users using 0 deposits
        await setAutomineBlocks(false)
        await chef.set(0, 10, rewarder2.address, true)
        await chef.connect(carol).deposit(0, 0, bob.address)
        await chef.connect(carol).deposit(0, 0, alice.address)
        await setAutomineBlocks(true)
        await advanceBlock()

        // we advance again by 100 seconds, harvest new reward token
        const afterFirstHarvest = await latest()
        await advanceToTime(afterFirstHarvest.toNumber() + 100)
        await setAutomineBlocks(false)
        await chef.connect(bob).harvest(0, bob.address)
        ...
        expect(await rewardToken2.balanceOf(alice.address)).to.equal(rewardsPerSecond.mul(100).mul(3).div(4))
        expect(await rewardToken2.balanceOf(bob.address)).to.equal(rewardsPerSecond.mul(100).div(4))

        expect(await rewarder2.pendingToken(0, alice.address)).to.equal(bn(0))
        expect(await rewarder2.pendingToken(0, bob.address)).to.equal(bn(0))
    })
