...
import { advanceBlockTo, advanceBlock, prepare, deploy, bn, ADDRESS_ZERO } from "./utilities"
...
describe("MasterChefV2", function () {
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", bn(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", bn(10)]],
      ["chef", this.MasterChef, [this.sushi.address, this.bob.address, bn(100), "0", "0"]],
    ])

    await this.sushi.transferOwnership(this.chef.address)
    await this.chef.add(100, this.lp.address, true)
    await this.chef.add(100, this.dummy.address, true)
    await this.lp.approve(this.chef.address, bn(10))
    await this.chef.deposit(0, bn(10))

    await deploy(this, [
      ["chef2", this.MasterChefV2, [this.chef.address, this.sushi.address, 1]],
      ["rlp", this.ERC20Mock, ["LP", "rLPT", bn(10)]],
      ["r", this.ERC20Mock, ["Reward", "RewardT", bn(100000)]],
    ])
    await deploy(this, [["rewarder", this.RewarderMock, [bn(1), this.r.address, this.chef2.address]]])
    await this.dummy.approve(this.chef2.address, bn(10))
    await this.chef2.init(this.dummy.address)
    await this.rlp.transfer(this.bob.address, bn(1))
  })
  ...
  describe("PendingSushi", function () {
    it("PendingSushi should equal ExpectedSushi", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      await this.rlp.approve(this.chef2.address, bn(10))
      let log = await this.chef2.deposit(0, bn(1), this.alice.address)
      await advanceBlock()
      let log2 = await this.chef2.updatePool(0)
      await advanceBlock()
      let expectedSushi = bn(100)
  ...
  describe("Deposit", function () {
    it("Depositing 0 amount", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      await this.rlp.approve(this.chef2.address, bn(10))
      await expect(this.chef2.deposit(0, bn(0), this.alice.address))
    ...
    it("Depositing into non-existent pool should fail", async function () {
      let err
      try {
        await this.chef2.deposit(1001, bn(0), this.alice.address)
      } catch (e) {
        err = e
      }
  ...
  describe("Withdraw", function () {
    it("Withdraw 0 amount", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      await expect(this.chef2.withdraw(0, bn(0), this.alice.address))
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of SUSHI and reward", async function () {
      await this.r.transfer(this.rewarder.address, bn(100000))
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      await this.rlp.approve(this.chef2.address, bn(10))
      ...
      let log2 = await this.chef2.withdraw(0, bn(1), this.alice.address)
      let expectedSushi = bn(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
  ...
  describe("EmergencyWithdraw", function () {
    it("Should emit event EmergencyWithdraw", async function () {
      ...
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      await this.rlp.approve(this.chef2.address, bn(10))
      await this.chef2.deposit(0, bn(1), this.bob.address)
      ...
      await expect(this.chef2.connect(this.bob).emergencyWithdraw(0, this.bob.address))
        ...
        .withArgs(this.bob.address, 0, bn(1), this.bob.address)
    })
  })
})
